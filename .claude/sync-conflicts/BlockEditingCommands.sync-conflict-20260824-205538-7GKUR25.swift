...
struct HyperlinkMarkdown {
    ...
    struct Match: Equatable {
        ...
        var target: Target? {
            switch kind {
            case .markdown:
                return HyperlinkMarkdown.target(from: urlText)
            case .wiki:
                return HyperlinkMarkdown.wikiTarget(from: urlText)
            }
        }

        /// Markdown punctuation to conceal (rendered state) or dim (revealed state).
        /// `[title](url)` conceals `[`, `](`, the url, and `)`; `[[path]]` conceals `[[` and `]]`.
        var syntaxRanges: [NSRange] {
            ...
            case .wiki:
                return [
                    NSRange(location: fullRange.location, length: 2),
                    NSRange(location: NSMaxRange(titleRange), length: 2),
                ]
            }
        }
    ...
    /// `[[vault/relative/path]]` — inner text may not contain brackets, `|` (alias syntax is
    /// unsupported and must stay plain text rather than mislink), or newlines.
    ...
    static func matches(in text: String) -> [Match] {
        ...
        let markdownMatches = regex.matches(in: text, range: fullRange).compactMap { match -> Match? in
        ...
        let wikiMatches = wikiRegex.matches(in: text, range: fullRange).compactMap { match -> Match? in
            guard match.numberOfRanges >= 2 else { return nil }

            let innerRange = match.range(at: 1)
            guard innerRange.location != NSNotFound,
                  NSMaxRange(innerRange) <= nsText.length else {
                return nil
            }

            let innerText = nsText.substring(with: innerRange)
            guard wikiTarget(from: innerText) != nil else { return nil }

            return Match(
                kind: .wiki,
                fullRange: match.range,
                titleRange: innerRange,
                urlRange: innerRange,
                title: innerText,
                urlText: innerText
            )
        ...
        guard !wikiMatches.isEmpty else { return markdownMatches }
        return (markdownMatches + wikiMatches).sorted { $0.fullRange.location < $1.fullRange.location }
    }
    ...
    /// Wiki-link inner text is a vault-relative path with the `.md` extension optional:
    /// `[[media/page-name]]` targets `media/page-name.md`.
    static func wikiTarget(from rawValue: String) -> Target? {
        let trimmed = rawValue.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return nil }

        let suffixed = trimmed.lowercased().hasSuffix(".md") ? trimmed : trimmed + ".md"
        guard let relativePath = normalizedVaultRelativePath(from: suffixed) else { return nil }
        return .vaultDocument(relativePath: relativePath)
    }
