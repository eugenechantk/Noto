...
//
//  Vendored from https://github.com/huggingface/swift-transformers (Sources/Tokenizers/Tokenizer.swift)
//  Tag: 1.3.3, commit 2fa33e1f5e7131a7fc64c28e6d161dcec0d24820. License: Apache-2.0.
//  See VendoredTokenizers/LICENSE-NOTICE.md.
//  Local modifications (trimmed to the BPE encode path needed for embeddings):
//  - removed `import Hub` / `import Jinja` (local Config shim lives in this module)
//  - removed chat-template support (Message/ToolSpec, ChatTemplateArgument,
//    applyChatTemplate, Jinja template cache, specialTokenAttributes)
//  - removed AutoTokenizer and the Hub/network loading paths
//  - removed non-BPE tokenizer registrations (Bert, Unigram, T5) and the
//    Llama PreTrainedTokenizer subclass; the class registry maps the BPE
//    family only, with PreTrainedTokenizer(Fast) hardwired to BPETokenizer
//
...
/// A tokenizer interface supporting encoding and decoding.
///
/// This is the main protocol that defines all tokenizer operations, including text
/// processing and special token handling.
...
/// A tokenizer implementation supporting pre-trained models from Hugging Face.
///
/// This class provides a tokenizer implementation that can be initialized from
/// Hugging Face configuration files and supports the standard tokenization operations:
/// normalization, pre-tokenization, model tokenization, and post-processing.
