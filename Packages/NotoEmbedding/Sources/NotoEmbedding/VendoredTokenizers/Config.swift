//
//  Vendored from https://github.com/huggingface/swift-transformers (Sources/Hub/Config.swift)
//  Tag: 1.3.3, commit 2fa33e1f5e7131a7fc64c28e6d161dcec0d24820. License: Apache-2.0.
//  See VendoredTokenizers/LICENSE-NOTICE.md.
//  Local modifications: removed `import Jinja` and the `jinjaValue()` accessor;
//  added a `Config(jsonData:)` initializer backed by JSONSerialization (replaces
//  the upstream Hub/yyjson loading path; no Hub, no yyjson, no network);
//  qualified Codable's Decoder/Encoder as Swift.Decoder/Swift.Encoder (the
//  vendored token Decoder protocol shares this module and shadows them).
//
//
//  Config.swift
//  swift-transformers
...
extension Config: Codable {
    ...
    private static func decodeTuple(_ decoder: any Swift.Decoder) -> Data? {
    ...
    private static func decodeArray(_ decoder: any Swift.Decoder) -> Data? {
        do {
    ...
    private static func decodeDictionary(_ decoder: any Swift.Decoder) -> Data? {
        do {
...
// MARK: - JSONSerialization shim (local addition, not in upstream)

/// Minimal replacement for swift-transformers' Hub/yyjson JSON loading.
/// Parses raw JSON data into a `Config` tree using Foundation's `JSONSerialization`.
///
/// `JSONSerialization` returns `NSDictionary`/`NSString` objects whose hashing and
/// equality are literal (binary) UTF-16 comparisons, so casting to `[NSString: Any]`
/// preserves vocab keys that would collide under Swift `String` canonical equivalence.
/// The existing `Config.init(_: [NSString: Any])` then converts the tree into
/// binary-distinct keyed storage.
public extension Config {
    init(jsonData: Foundation.Data) throws {
        let parsed = try JSONSerialization.jsonObject(with: jsonData)
        guard let dictionary = parsed as? [NSString: Any] else {
            throw ConfigError.typeConversionFailed(value: "non-dictionary JSON root", targetType: Config.self)
        }
