...
function DesignCanvas({ children, minScale, maxScale, style, stateFile }) {
  // Sibling canvases that live in the same folder (e.g. an iteration of this
  // file) can pass a distinct `stateFile` so their saved order/titles/hidden
  // state don't collide. Defaults to the shared sidecar name.
  const STATE_FILE = stateFile || DC_STATE_FILE;
  ...
  React.useEffect(() => {
    ...
    const t = setTimeout(() => {
      window.omelette?.writeFile(STATE_FILE, JSON.stringify({ sections: state.sections })).catch(() => {});
    }, 250);
