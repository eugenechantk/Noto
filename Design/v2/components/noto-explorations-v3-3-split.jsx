// v3-split · Digital article — split top/bottom Liquid Glass nav.
// Continues the Tabs lineage but splits the chrome across two glass rows:
//   TOP    [back/fwd]   [   title pill (tab trigger)   ]   [more]
//   BOTTOM [shelf]      [search]   [today]    [+ new note]
// Tapping the title pill opens the same tab switcher used in v3-tabs.
//
// In editing state, the keyboard accessory toolbar replaces the bottom nav
// (since the keyboard occupies that space). In switcher state, the bottom
// row is replaced by a single "Done" pill.
//
// Pulls shared chrome: ArticleIcon, NOTE, IOSDevice, IOSKeyboard,
// NotoToolbarArticle — all on window.

function Exp03ArticleV3Split({ state = 'reading' }) {
  const BG     = '#0E1116';
  const INK    = '#ECECEE';
  const HEAD   = '#FFFFFF';
  ...
  // ─── Top glass bar: back/fwd · title pill · more ────────────
  const FloatingTopBar = ({ titleOverride = null, accent = false }) => (
    <div style={{
      ...
      {/* Back / forward — paired glass capsule */}
      <div style={{
        ...glassStyle,
      ...
      {/* Title pill — tap to open tab switcher */}
      <div style={{
        ...glassStyle,
        flex: 1, minWidth: 0, height: 38, borderRadius: 999,
        display: 'flex', alignItems: 'center', gap: 8,
        padding: '0 12px 0 14px', pointerEvents: 'auto',
  ...
  // ─── Bottom glass nav: shelf · search · today · + new note ──
  const FloatingBottomNav = () => (
    <div style={{
      ...
      <div style={{
        ...
        <ArticleIcon name="sidebar" color={HEAD} size={19} />
      </div>

      {/* Search + Today — paired capsule */}
      <div style={{
        ...glassStyle,
        ...
        <div style={{ width: 0.5, height: 20, background: 'rgba(255,255,255,0.14)' }} />
        <div style={{
          width: 36, height: 36, borderRadius: 999,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}>
          <ArticleIcon name="today" color={HEAD} size={17} />
        </div>
      </div>

      <div style={{ flex: 1 }} />

      {/* + New note — accent pill */}
      <div style={{
        ...glassStyle,
        height: 44, borderRadius: 999,
        padding: '0 18px 0 14px', gap: 8,
        ...
        <span style={{ color: HEAD, fontFamily: SANS, fontSize: 14, fontWeight: 600 }}>New note</span>
      </div>
    </div>
  );
  ...
  // ─── Tab switcher (same model as v3-tabs) ──────────────────
  const TabView = () => (
    <div style={{
      flex: 1, minHeight: 0, overflow: 'hidden',
  ...
  // ─── Bottom in switcher = single Done pill ─────────────────
...
Object.assign(window, { Exp03ArticleV3Split });
