// v3-tabs · Digital article — every note is a tab; title is the switcher.
// Continues the Liquid Glass lineage. The top bar is reduced to THREE capsules:
//   [shelf]   [   active tab pill   ]   [more]
// There is no back/forward — navigating between notes happens through the tab
// switcher, opened by tapping the title pill. The switcher (TabView) shows a
// scrollable column of card-shaped tab previews; each card has a close ×,
// title, snippet, footer meta, and a soft glass surface. A floating "+ New"
// pill sits at the bottom to open a fresh note as a new tab. Done dismisses.
//
// Pulls shared chrome from v3-3: ArticleIcon, NOTE, IOSDevice, IOSKeyboard,
// NotoToolbarArticle — all on window.

function Exp03ArticleV3Tabs({ state = 'reading' }) {
  const BG     = '#0E1116';
  const INK    = '#ECECEE';
  const HEAD   = '#FFFFFF';
  ...
  // Shared open-tab roster — used by both the title pill (active = first) and
  // the switcher view (renders all of them).
  const TABS = [
    {
      id: 't1', active: true,
      title: 'How to Build Strong AI Products',
      snippet: 'A field guide for founders shipping AI features in 2026 — what separates the products that compound from the ones that flame out.',
      folder: 'Captures', meta: 'edited 2h ago', tint: 'linear-gradient(135deg, #FF6A2E 0%, #C2185B 55%, #4A148C 100%)',
    },
    {
      id: 't2', active: false,
      title: 'Weekly review · Mar 11',
      snippet: 'Wrapped the AC pricing experiment. Three things to follow up on next week — landed on a v2 of the deposit model.',
      folder: 'Journal', meta: 'edited yesterday', tint: 'linear-gradient(135deg, #1F6FEB 0%, #163172 100%)',
    },
    {
      id: 't3', active: false,
      title: 'Sahil — compounding deposits',
      snippet: 'Quoting the substack post about durable AI products. The user data is the moat; the model is a swap-out commodity.',
      folder: 'Highlights', meta: 'edited 3d ago', tint: 'linear-gradient(135deg, #2EB67D 0%, #064E3B 100%)',
    },
    {
      id: 't4', active: false,
      title: 'Field guide outline',
      snippet: '1. The compounding loop. 2. The three deposits. 3. The AI-model-layer story. 4. Field-guide intro.',
      folder: 'Drafts', meta: 'edited 5d ago', tint: 'linear-gradient(135deg, #E6B62A 0%, #7C4A00 100%)',
    },
    {
      id: 't5', active: false,
      title: 'Pricing experiments Q1',
      snippet: 'Three lever ideas: per-seat with usage cap, hybrid base+usage, and a metered drip. Need to validate with sales.',
      folder: 'Projects', meta: 'edited Mar 4', tint: 'linear-gradient(135deg, #8B5CF6 0%, #312E81 100%)',
    },
  ];

  // ─── Floating glass top bar ─────────────────────────────────
  // No back/forward — title pill IS the tab switcher trigger.
  const FloatingTopBar = ({ titleOverride = null, accent = false }) => (
    <div style={{
      ...
      {/* Active tab pill — tap to open switcher */}
      <div style={{
        ...glassStyle,
        ...
        pointerEvents: 'auto',
        // when in switcher view, indicate "selected" by tinting the border
        ...(accent ? { border: '0.5px solid rgba(255,106,46,0.55)' } : {}),
      }}>
        ...
        <span style={{
          ...
          borderRadius: 6, fontFamily: SANS, fontSize: 11, fontWeight: 600,
          color: HEAD,
  ...
  // ─── Reading body (same article copy as v3-glass) ───────────
  const ArticleBody = () => (
    <div style={{
  ...
  // ─── Tab switcher view ──────────────────────────────────────
  // Card per tab; the active one is highlighted; close × on each; floating
  // bottom toolbar with "+ New note" pill + tab count + Done.
  const TabView = () => (
    <div style={{
      flex: 1, minHeight: 0, overflow: 'hidden',
      ...
      {/* cards stack — vertical scroll feel (no actual scroll in mock) */}
      <div style={{
        display: 'flex', flexDirection: 'column', gap: 10,
        paddingBottom: 90, // room for floating bottom dock
        overflow: 'hidden',
      }}>
        {TABS.map((t) => (
          <div key={t.id} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{
                  marginTop: 2,
                  color: MUTED, fontFamily: SANS, fontSize: 11,
                  display: 'flex', alignItems: 'center', gap: 6,
                }}>
                  ...
                  {t.active && (
                    <>
                      <span style={{ opacity: 0.4 }}>·</span>
                      <span style={{ color: ACCENT, fontWeight: 600 }}>Active</span>
                    </>
                  )}
                </div>
              </div>
            ...
            {/* snippet — limited to 2 lines */}
            <div style={{
              color: INK, fontFamily: SANS, fontSize: 13, lineHeight: 1.45,
              opacity: 0.78,
      ...
      <div style={{
        ...
        <div style={{
          ...
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke={HEAD} strokeWidth="1.8" strokeLinecap="round">
  ...
  const showKeyboard = state === 'editing';
...
Object.assign(window, { Exp03ArticleV3Tabs });
