...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.noto", category: "TodayNotesService")

struct TodayNotesService {

    // MARK: - Root Block

    /// Ensure the "Today's Notes" root block exists. Creates it on first launch.
    @MainActor
    static func ensureRoot(context: ModelContext) -> Block {
    ...
    @MainActor
    static func buildHierarchy(root: Block, for date: Date, context: ModelContext) -> Block {
        let cal = calendar
    ...
    @MainActor
    static func ensureToday(context: ModelContext, date: Date = Date()) -> Block {
    ...
    static func buildSteps(for date: Date, calendar cal: Calendar = calendar) -> [BuildStep] {
        ...
        let mondayYear = cal.component(.year, from: monday)
    ...
    static func formatWeek(number: Int, monday: Date, sunday: Date, calendar cal: Calendar) -> String {
    ...
    static func formatDay(_ date: Date, calendar cal: Calendar = calendar) -> String {
        let formatter = DateFormatter()
        formatter.locale = Locale(identifier: "en_US_POSIX")
    ...
    static func mondayOfWeek(containing date: Date, calendar cal: Calendar) -> Date {
    ...
    static func weekNumberInMonth(for monday: Date, calendar cal: Calendar) -> Int {
